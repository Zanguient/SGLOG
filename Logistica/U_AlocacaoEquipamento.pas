  ...
  Sql := 'SELECT f.cod_fazenda "Codigo",                  ' +
   ...
   If valor <> null then begin
     if not (QR_Fazenda.State in [dsInsert,dsEdit]) then
      ...
      QR_FazendaFAZENDA.AsString         := F_BuscaAvancada.Query.FieldByName('Descrição').AsString;
  ...
  inherited;
   ...
   If valor <> null then begin
     if not (QR_Fazenda.State in [dsInsert,dsEdit]) then
      ...
      QR_FazendaTIPOOCANA.AsString     := F_BuscaAvancada.Query.FieldByName('Descrição').AsString;
...
begin
  inherited;
   ...
   if APrevFocusedItem.Caption = 'Código' then
   begin
       if (QR_Fazenda.State in [dsInsert,dsEdit]) then
       begin
         try
...
begin
  ...
  Sql := Sql + ' ORDER BY equipamento.descricao                                                 ';
   ...
   valor := F_BuscaAvancada.Buscar(sql,1,'Busca de Equipamentos(Caminhões)');
   If valor <> null then begin
     if not (QR_EquipamentosFixos.State in [dsInsert,dsEdit]) then
      ...
      QR_EquipamentosFixosEQUIPAMENTO.AsString     := F_BuscaAvancada.Query.FieldByName('Veículo').AsString;
   end;
end;
...
begin
  inherited;
  Sql := 'SELECT cod_tipocarga "Código", descricao "Descrição" ' +
         '  FROM tipo_carga                                   ' +
         ' WHERE ativo = ''S''                                ' +
         ' &Macro                                             ' +
   ...
   If valor <> null then begin
     if not (QR_TemposCarregamento.State in [dsInsert,dsEdit]) then
      ...
      QR_TemposCarregamentoDESCRICAO.AsString     := F_BuscaAvancada.Query.FieldByName('Descrição').AsString;
    ...
    begin
      ...
      begin
       try
         ...
         //Pega o peso pradão do tipo de carga
         ...
         begin
          ...
          on e:exception do
          begin
              Application.MessageBox(Pchar('Erro ao localizar tipo de carga, verificar se existe o código '
  ...
  Sql := 'SELECT cod_tipocarga "Código", descricao "Descrição" ' +
   ...
   If valor <> null then begin
     if not (QR_TemposRealizados.State in [dsInsert,dsEdit]) then
      ...
      QR_TemposRealizadosDESC_TIPOCARGA.AsString     := F_BuscaAvancada.Query.FieldByName('Descrição').AsString;
...
begin
  ...
  begin
    Application.MessageBox('Selecione um equipamento que está em Parada.','Aviso',mb_ok+MB_ICONINFORMATION);
    Abort;
  end;
...
begin
  ...
  begin
    Application.MessageBox('Selecione um equipamento que esta em operação.','Aviso',mb_ok+MB_ICONINFORMATION);
    Abort;
  end;
...
begin
  ...
  begin
    Application.MessageBox('Selecione um equipamento que está em operação.','Aviso',mb_ok+MB_ICONINFORMATION);
    Abort;
  end;
...
begin
  ...
  begin
    Application.MessageBox('Computador não autorizado para fazer exclusão de alocações.','Aviso',MB_OK+MB_ICONWARNING);
    Abort;
  end;

   if Application.MessageBox(PChar('Deseja excluir a alocação do equipaemnto '+QR_AlocacaoCOD_EQUIPAMENTO.AsString),'Confirmação',mb_yesno+mb_iconquestion) = idyes then
   begin
    QR_Alocacao.Delete;
   end;
end;
...
begin
  ...
  if not fn_computador_logistica(dm.QR_UnidadeCOD_GRUPO.AsInteger,
                                 ...
                                 fnc_computador_logado) then
  begin
    Application.MessageBox('Computador não autorizado para fazer finalização de alocações.','Aviso',MB_OK+MB_ICONWARNING);
    Abort;
  end;

   if Application.MessageBox(PChar('Deseja finalizar a alocação do equipaemnto '+QR_AlocacaoCOD_EQUIPAMENTO.AsString),'Confirmação',mb_yesno+mb_iconquestion) = idyes then
   begin
...
begin
  ...
  if lvCarregOp.ItemIndex = -1 then
  begin
    Application.MessageBox('Selecione um equipamento em Operação.','Aviso',mb_ok+MB_ICONINFORMATION);
    Abort;
  end;
     ...
     SQL:= 'SELECT e.cod_equipamento "Equipamento", e.descricao "Descricao", t.tipo_cct "Tipo" ' +
      ...
      begin
         if Application.MessageBox(PChar('Deseja alocar o equipamento '
                                         ...
                                         +' e Fazenda '+QR_FrenteFazendasCOD_FAZENDA.AsString),'Confirmação',MB_YESNO+MB_ICONQUESTION) = IDYES then
         begin
            with TOraQuery.Create(Self) do
            begin
              Session := DM.Sessao;
              ...
              if not IsEmpty then
              begin
                Application.MessageBox(PChar('Equipamento já alocado para a Frente '+FieldByName('cod_frente').AsString
