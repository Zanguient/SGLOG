...
begin
  ...
  begin
    ...
    vcod_frente  := Informacao.cod_frente;
    vcod_fazenda := Informacao.cod_fazenda;
...
begin
 ...
 begin
    if Application.MessageBox(PChar('Deseja retornar o equipamento '+FloatToStr(veiculo)+' em Operação?'),'Confirmação',MB_YESNO+MB_ICONQUESTION) = IDNO then
        ...
        begin
          Close;
          Session := dm.Sessao;
          SQL.Text := 'UPDATE PARADA_EQUIPAMENTO SET FIM_PARADA = SYSDATE ' +
          ...
          ParamByName('cod_unidade').AsFloat := DM.QR_UnidadeCOD_UNIDADE.AsFloat;
          ParamByName('cod_safra').AsFloat := DM.QR_SafrasCOD_SAFRA.AsFloat;
          ParamByName('cod_equipamento').AsFloat := Veiculo;
          ExecSQL;
          Free;
        end;
 end;
 ...
 if Verificar_Formulario_Criado(TF_DisponibilidadesFrentes, False) then
    TF_DisponibilidadesFrentes(owner).Cria_frentes;
end;
...
begin
   ...
   begin
     ...
     if vTipoEquipamento = 'C' then //Quando for caminhão
     begin
        with TOraQuery.Create(Self) do
        begin
          Close;
