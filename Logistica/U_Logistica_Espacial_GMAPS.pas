...
// Data : 07/02/2012
// Desenvolvedor : P1
// Descrição Função/Procedure/Tela : Logistica espacial, possui a visualização das frentes de colheitas e
// rotas da usina até a fazendas/frentes, é possivel fazer o monitoramento dos equipamentos que estão
...
begin
  ...
  if informacoesAutomaticas then
    begin
      rzBitBtnInfoAutomaticas.Hint := 'Desativar informações automáticas dos equipamentos';
      rzBitBtnInfoAutomaticas.ImageIndex := 391;
    end
  else
    begin
      rzBitBtnInfoAutomaticas.Hint := 'Ativar informações automáticas dos equipamentos';
...
begin
  if parado then
    begin
      ...
      rzBitbtnStatusAtualizacao.Hint := 'Status Atualização: Parado';
    end
  else if atualizacao then
    begin
      rzBitbtnStatusAtualizacao.ImageIndex := 2275;
      rzBitbtnStatusAtualizacao.Hint := 'Status Atualização: Atualizando Informações';
    end
  else if operacao then
    begin
      rzBitbtnStatusAtualizacao.ImageIndex := 2276;
      rzBitbtnStatusAtualizacao.Hint := 'Status Atualização: Operação';
    end;
end;
...
begin
  ...
  if iconeOperacaoAgricola then
    begin
      RzBitBtnIconeOperacao.Hint := 'Desativar Ícone Operação Agrícola';
      RzBitBtnIconeOperacao.ImageIndex := 988;
    end
  else
    begin
      RzBitBtnIconeOperacao.Hint := 'Ativar Ícone Operação Agricola';
