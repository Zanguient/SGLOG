...
type
  TF_GravarAlocacao = class(TForm)
    ...
    Label3: TLabel;
    Label4: TLabel;
    btnPesquisa_Veiculos: TSpeedButton;
    ...
    procedure btnPesquisa_VeiculosClick(Sender: TObject);
...
begin
  ...
  begin
    Application.MessageBox('Usuário não autorizado para inserir alocações.','Aviso',MB_OK+MB_ICONWARNING);
    Abort;
  end;
...
procedure TF_GravarAlocacao.btnPesquisa_VeiculosClick(Sender: TObject);
var SQL : String;
    valor : Variant;
begin
//   showmessage('lembrar de habilitar recurso que desabilita o botao gravar');
...
procedure TF_GravarAlocacao.wwDBEdit5KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = VK_F9 then
    btnPesquisa_Veiculos.Click;
end;
