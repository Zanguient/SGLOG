...
Begin
   ...
   // Verifica se a maior distancia é maior que a qtde_numeros_regua
   ...
   End
   // A maior distancia é menor que a qtde_numeros_regua
   else Begin
  ...
  cria_regua(QR_PistasMAIOR_DISTANCIA_REGUA.AsFloat);
   ...
   Begin
      ...
      begin
         ...
         Begin
            //Pegar a posição do Scroll para voltar a barra de rolagem depois de criar os componentes
   ...
   Begin
     // Cria a Pista cabeçalho
      ...
      //Cria o botão de informações da pista
      ...
      begin
        ...
        begin
          Application.MessageBox(Pchar('Componente Já está criado, verificar alocação do equipamento'+QR_VeiculosCOD_EQUIPAMENTO.AsString),'Aviso',MB_OK+MB_ICONERROR);
        ...
        //##### DESCARREGANDO ######
         ...
         //                           'Data Saída  : '+QR_VeiculosDATA_SAIDA.AsString;
...
Begin
   ...
   QR_Pistas.Close;
   QR_Pistas.Open;
   //Paga a pagina que será criada, ou seja, a pagina que não está ativa
   //Essa variavél será usada nos nomes de todos os componentes criados da PISTA
   if RzPageControl2.ActivePage = Page1 then
      v_pagecontrol := '2'
   else v_pagecontrol := '1';

   // Cria a regua para demonstrar a maior distância em Km.
   ...
   //Cria no Panel que está na pagina que não está ativa
   if RzPageControl2.ActivePage = Page1 then
...
//   TPista.Height   := TabSheet1.Height - Panel3.Height - Panel6.Height;
   ...
   Begin
      ...
      begin
        Application.MessageBox('Não foi Possível Encontrar a Maior Distância para a Formação das Pistas!', 'Atenção!', MB_ICONERROR + MB_OK);
        Close;
        Abort;
      end;
      ...
      //atualizao o traço para a pista maior
      if TPista_estrada_ida.Width >  TPIsta_estrada_volta.Width then
        TPista_Traco.Width := TPista_estrada_ida.Width - 4
      else
        TPista_Traco.Width := TPista_estrada_volta.Width - 4;
      ...
      //Cria Informações Frente Cabeçalho
      ...
      Begin
         ...
         if QR_VeiculosTEMPOREAL.AsString = 'S' then
            TVeiculoImagePista.informacaoBalao.RealPrevisto := 'Posição Real'
            else
              TVeiculoImagePista.informacaoBalao.RealPrevisto := 'Posição Previsto';
         ...
         if QR_ALERTA_ALOCACAO.IsEmpty then
           TVeiculoImagePista.informacaoBalao.Alarmes.Add('-> Não possui Avisos.')
         else
           ...
           Begin
              ...
              // Le o próximo alerta do veiculo
              QR_ALERTA_ALOCACAO.Next;
           End;

         //Marcelo Hirata - 07/06/2011
         //Verifica se a alocação possui alertas
         ...
         if possuiAlertas then
           ...
           TVeiculoImagePista.informacaoBalao.Alertas.Add('-> Não possui alertas.');
         ...
         //#####################################################################
         // P3 OS VEICULOS EM PARADAS SEMPRE TERÃO ALERTAS POR ESSE MOTIVO ESTAO COMTEMPLADOS APENAS PARA OS ALERTAS.

         If QR_VeiculosSITUACAO.Value = 'I' Then
         Begin
           //Incrementa informação quantidade indo da pista
   ...
   //                      Teste.Picture.Graphic := btnPNG_Direita_Vazio_Alerta_Selecionado.ImageNormal;
              ...
              else //daki para baixo os equipamentos que não  estão selecionados
              begin
                  TVeiculoImagePista.informacaoBalao.TipoImagem := 0;
                  //Se a alocação possuir alertas
                  if possuiAlertas then
         ...
         begin
           ...
           else //daki para baixo é do Real
           begin
             // Verifica se é para localizar o veículo
             if TVeiculoImagePista.Tag = Rveiculo Then
             begin
         ...
         begin
           ...
           //verifica se é para gerar pelo tempo real
           if QR_VeiculosTEMPOREAL.AsString = 'S' then
           begin
              if TVeiculoImagePista.Tag = Rveiculo then
              begin
         ...
         case cbVisualizarCodigoVeiculo.ItemIndex of
           1 : begin
                   //Cria o panel com o código do equipamento
   ...
   //depois que tudo estiver criado, percebemos que a performance melhorou bastante
   TPista.Visible := true;
   //Volta a posição original do Scroll, para não ter que rolar na barra de rolagem
...
begin
  inherited;
  if Application.MessageBox(Pchar('Deseja realmente encerrar a parada para o equipamento '+QR_ParadoCOD_EQUIPAMENTO.AsString+'?'),'Confirmação',MB_YESNO+MB_ICONQUESTION) = IDYES then
  begin
    QR_Parado.Edit;
...
begin
  inherited;
   ...
   begin
      //Fecha o form e desaloca da memória
...
//  Delete(CodigoVeiculo,1,8);
  ...
  if Posicao = 0 then
    begin
      Application.MessageBox('Não foi encontrada uma alocação para esse equipamento!','Alocação não encontrada.',MB_ICONERROR + MB_OK);
      Abort;
    end
  else
    begin
      VeiculoDaPista := StrToInt(Copy(CodigoVeiculo,1,Posicao - 1));
      ransferirdeEmpresa1.Caption := 'Transferir o Veículo ' + IntToStr(VeiculoDaPista) + ' de Empresa';
      ransferirdeFazenda1.Caption := 'Transferir o Veículo ' + IntToStr(VeiculoDaPista) + ' de Fazenda';
    end;
end;
...
begin
  ...
  Sql := Sql + ' ORDER BY equipamento.descricao                                                 ';
   ...
   If valor <> null then begin
      ...
      wwDBEdit6.Text := F_BuscaAvancada.Query.FieldByName('Veículo').AsString;
...
begin
  inherited;
   ...
   begin
      with TOraQuery.Create(Self)do
      begin
        SQL.Text := '  SELECT equipamento.cod_equipamento "Código", equipamento.descricao "Veículo"  '+
        ...
        if not IsEmpty then
        begin
          wwDBEdit6.Text := FieldByName('Veículo').AsString;
          rveiculo := StrToFloat(wwDBEdit5.Text);
          btnAtualizarClick(Self);
        end
        else Application.MessageBox('Veículo não  encontrado','Aviso',mb_ok+MB_ICONEXCLAMATION);
        Free;
      end;
    end
    else
